...
export TEST_COMPACT_CHUNK_IDS="${TEST_COMPACT_CHUNK_IDS:-5000}"
...
if [ -z "${TEST_WALLET_SLOTS:-}" ]; then
  if [ "${CHAIN}" = "base" ]; then
    export TEST_WALLET_SLOTS="2"
  else
    export TEST_WALLET_SLOTS="5"
  fi
else
  export TEST_WALLET_SLOTS
fi
export TEST_PARALLEL_KA_BATCH_SIZE="${TEST_PARALLEL_KA_BATCH_SIZE:-5}"
export TEST_BATCH_DELAY_MS="${TEST_BATCH_DELAY_MS:-5000}"
...
export TEST_INSUFFICIENT_FUNDS_MAX_RETRIES="${TEST_INSUFFICIENT_FUNDS_MAX_RETRIES:-5}"
export TEST_INSUFFICIENT_FUNDS_COOLDOWN_MS="${TEST_INSUFFICIENT_FUNDS_COOLDOWN_MS:-65000}"
