...
function getNodeWallet(nodeId, walletSlot = null) {
  ...
  const prefixedPublic = process.env[`JS_TESTNET_GNOSIS_NODE${normalizedNodeId}_W${resolvedWalletSlot}_PUBLIC_KEY`];
  const prefixedPrivate = process.env[`JS_TESTNET_GNOSIS_NODE${normalizedNodeId}_W${resolvedWalletSlot}_PRIVATE_KEY`];
  const legacyPublic = process.env[`JS_TESTNET_GNOSIS_NODE${normalizedNodeId}_PUBLIC_KEY`];
  const legacyPrivate = process.env[`JS_TESTNET_GNOSIS_NODE${normalizedNodeId}_PRIVATE_KEY`];
    ...
    for (let currentIndex = 0; currentIndex < nodesToRun.length; currentIndex++) {
      ...
      const processKnowledgeAsset = async (kaNumber, walletSlot) => {
        ...
        try {
          ...
          const sparql = TEST_COMPACT_CHUNK_MODE
          ...
          const queryResult = await Promise.race([
            DkgClient.graph.query(sparql, 'SELECT'),
            new Promise((_, reject) =>
